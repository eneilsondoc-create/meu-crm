...
try:
    ...
    st.error("Aguarde... Instalando dependências ou verifique o arquivo requirements.txt")
    st.stop()
...
if pagina == "💰 Lançamentos":
    ...
    df_v = carregar_dados(VENDAS_FILE, ["Data", "Cliente", "Descrição", "Valor", "Recebido"])
    ...
    with st.expander("➕ Nova Venda", expanded=True):
        ...
        if st.button("💾 Salvar Venda", type="primary", use_container_width=True):
            novo = pd.DataFrame([{"Data": data_v.strftime("%d/%m/%Y"), "Cliente": cli_v, "Valor": str(val_v), "Recebido": rec_v}])
    ...
    if not df_v.empty:
        ...
        idx_v = st.number_input("ID da linha para apagar:", min_value=0, max_value=len(df_v)-1, step=1)
...
# --- PÁGINA 2: DESPESAS ---
    ...
    with st.expander("➕ Nova Despesa", expanded=True):
        c1, c2 = st.columns(2)
        with c1:
            dt_d = st.date_input("Data", datetime.now())
            nome_d = st.text_input("Descrição")
        with c2:
            v_d = st.number_input("Valor (R$)", min_value=0.0, format="%.2f")
            p_d = st.selectbox("Pago?", ["Sim", "Não"])

        if st.button("💾 Salvar Despesa", type="primary", use_container_width=True):
            novo_d = pd.DataFrame([{"Data": dt_d.strftime("%d/%m/%Y"), "Despesa": nome_d, "Valor": str(v_d), "Pago": p_d}])
    ...
    if not df_d.empty:
        ...
        idx_d = st.number_input("ID da linha para apagar:", min_value=0, max_value=len(df_d)-1, step=1)
...
# --- PÁGINA 3: RESUMO GERAL ---
    ...
    if not df_v.empty or not df_d.empty:
        ...
        fig = go.Figure()
        fig.add_trace(go.Bar(x=meses, y=res_v, name='Recebido', marker_color='#2ecc71', text=res_v, textposition='auto'))
        fig.add_trace(go.Bar(x=meses, y=res_d, name='Pago', marker_color='#e74c3c', text=res_d, textposition='auto'))
